...
async fn delete_workout_template_(event: Request, test_db: Option<DBPool>) -> Result<Response<Body>, Error> {
...
async fn main() -> Result<(), Error> {
    ...
    let handler = service_fn(|event| delete_workout_template_(event, None));
    run(handler).await
}

...
async fn create_workout_template(event: Request, test_db: Option<DBPool>) -> Result<Response<Body>, Error> {
...
async fn main() -> Result<(), Error> {
    ...
    let handler = service_fn(|event| create_workout_template(event, None));
    run(handler).await
}
